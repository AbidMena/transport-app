 ...
 if (!$this->upload->do_upload('car_image')){
 	...
 	$result =$this->home->update_car_details11($data);
			...
			if($result) {
				...
				$this->session->set_flashdata('message', array('message' => 'Error Occured','class' => 'success'));  
			}
		redirect(base_url().'admin/view_car');	
		}
...
--------------------------------------------------------*/ 
	public function round_package_add(){
	    ...
	    $data=$_POST;
	    $res1= $this->home->getRoundPackageone($data);
	    if(!$res1){
		...
		$this->session->set_flashdata('message', array('message' => 'Already Existing package','class' => 'danger')); 

	}
	redirect(base_url().'admin/add_round_package');
	}  	
...
--------------------------------------------------------*/	
	public function outstation_add_package(){
        ...
        if($res=="success") {
			...
			$this->session->set_flashdata('message', array('message' => 'Already Exist The Car or Package ','class' => 'success'));  
		}
