...
public function about()
   ...
   $this->load->view('template', $template);
    ...
    public function Forget_Password() {
        ...
        $result = $this->Model_cab->forgetpassword($request['email']);
		...
		/*$template['page_title'] ="header"; 
	    $template['page'] ="header";

		//$request = file_get_contents("php://input");
         $data = $_post;//json_decode($request);
         print_r($data);
...
echo $book;
		...
		public function book_point()
	{
		 ...
		 $data = $_POST;
		 $username = $this->session->userdata('username');
		 $book=$this->Model_cab->booking_point($data,$username);
		 echo $book;
	}
...
$this->pagination->initialize($config);
  ...
  $page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
        ...
        $data["results"] = $this->Model_cab->fetch_countries1($config["per_page"], $page);
	...
	{ 		$data=$_POST;
		...
		$template['page_name'] = "success_page";
	        ...
	        public function saveaddress(){
	        	$data = $_POST;
	        	// print_r($data);
	        	$res = $this->Model_cab->insert_address($data);
	        	
	        	if($res==success)
			    {
							
			        	$result = array('status'  => 'success','message'  => 'Successfully Inserted');
			 	}
